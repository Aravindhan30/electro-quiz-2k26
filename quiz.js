...
// ── Google Apps Script Backend URL — PASTE YOUR DEPLOYED URL HERE ──
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxVdQByrqrBqpvcPvcvfb5pnJMtVzRHFeRfAyWWwKsf3lWvvRXp0Tt2RSS7a6oNcNfl4w/exec';
...
function submitQuiz() {
  ...
  saveToBackend(correct, state.questions.length, timeTaken);

  showResults(correct, state.questions.length, timeTaken);
}

function saveToBackend(score, total, timeTaken) {
  if (!SCRIPT_URL || SCRIPT_URL === 'YOUR_APPS_SCRIPT_URL') return;
  try {
    const data = {
      action: 'submitQuiz',
...
window.addEventListener('beforeunload', (e) => {
  if (state.questions.length > 0 && !state.submitted) {
    // Auto-submit answers to Apps Script before leaving
    let correct = 0;
    state.questions.forEach((q, i) => {
      if (state.answers[i] === q.answer) correct++;
    });
